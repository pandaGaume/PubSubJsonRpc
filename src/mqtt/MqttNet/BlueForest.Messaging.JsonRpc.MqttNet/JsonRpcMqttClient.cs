...
{
    public class JsonRpcMqttClient<T> : IMqttApplicationMessageReceivedHandler, IDisposable
        where T:class
    {
        ...
        public async Task BuildAsync(JsonRpcMqttSettings settings, int routeIndex = 0)
        {
            ...
            if (settings.ProcedureCallSession.HasValue)
            {
                ...
                if (controlClient != null)
                {
                    ...
                    var listenTopic = new MqttRpcTopic(route.Path, route.To, settings.Name);
